...
const Rentbikereviews = () => {
  ...
  const sendId = async () => {
    try {
      const res = await fetch(
        "https://bike-rental-portal-k5bj.onrender.com/sendReviewRentBikeId",
        {
          method: "POST",
          headers: {
  ...
  const reviewBikeData = async () => {
    try {
      const res = await fetch(
        "https://bike-rental-portal-k5bj.onrender.com/getRentBikeReviews",
        {
          method: "GET",
        }
      );
  ...
  const getallreviews = async () => {
    try {
      const res = await fetch(
        "https://bike-rental-portal-k5bj.onrender.com/getallreviewsforselectedrentbike",
        {
          method: "GET",
        }
      );
  ...
  const submitReviews = async (e) => {
    ...
    const res = await fetch(
      "https://bike-rental-portal-k5bj.onrender.com/postrentbikereviews",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  return (
    <>
      ...
      <div className="reviewsdiv">
        <img
          src={renttbikesData.filePath}
          alt=""
          style={{ width: "80%", height: "70%" }}
        />
        <h4>
          <b>{renttbikesData.brand}</b>
        </h4>
          ...
          <div className="reviewsli" key={allrenttbikeReviews._id}>
            <ul>
              <li style={{ wordSpacing: "10px" }}>
                {allrenttbikeReviews.name} :- {allrenttbikeReviews.comments}
              </li>
            </ul>
          </div>
        ))}
