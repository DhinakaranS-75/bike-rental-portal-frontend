...
const Rentbikecart = () => {
  ...
  const getCartData = async () => {
    try {
      const res = await fetch(
        "https://bike-rental-portal-k5bj.onrender.com/getRentCartData",
        {
          method: "GET",
        }
      );
  ...
  const handlePayMethod = (itemsPrice, token) => {
    return fetch("https://bike-rental-portal-k5bj.onrender.com/stripeRentPay", {
      method: "POST",
      headers: {
  ...
  const updateDataBase = () => {
    return fetch(
      "https://bike-rental-portal-k5bj.onrender.com/updateRentDataBase",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
