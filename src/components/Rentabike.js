...
const Rentabike = () => {
  ...
  const allRentBikes = async () => {
    try {
      ...
      const res = await fetch(
        "https://bike-rental-portal-k5bj.onrender.com/getRentBikeData",
        {
          method: "GET",
        }
      );
  ...
  const proceedToCart = async (e) => {
    e.preventDefault();
    console.log("proceedToCart clicked");
  
    let itemId = e.target.id;
    console.log("itemId:", itemId);
    console.log("rentHours:", rentHours);
  
    const res = await fetch(
      "https://bike-rental-portal-k5bj.onrender.com/addrentcartocart",
      {
        method: "POST",
        headers: {
  ...
  const searchTextBtn = async () => {
    const res = await fetch(
      "https://bike-rental-portal-k5bj.onrender.com/searchRentBike",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const getSearchData = async () => {
    try {
      const res = await fetch(
        "https://bike-rental-portal-k5bj.onrender.com/rentbikesearchCategory",
        {
          method: "GET",
        }
      );
      ...
      <div className="rentbikebiked">
        {rentBikesData.map((rentBikesData, index) => [
          ...
          <div className="specsDivRentbike" key={new Date()}>
            ...
            <p style={{ color: "red" }}>
              Availibility : {rentBikesData.availability + " hours"}
            </p>
            ...
            <div style={{ display: "flex", gap: "15px" }}>
              <button className="bikedbtn">
                <NavLink
                  className="nav-link"
                  to={{
                    pathname: "/rentbikereviews",
                    state: { id: rentBikesData._id },
                  }}
                >
                  Bike Reviews
                </NavLink>
          ...
          <div className="formDivRentbike" key={index}>
            <form method="POST">
              <h3>
                Before click on proceed please enter for how many hours do you
                want to rent the bike
              </h3>
              <br />
              <label htmlFor="lname">Rent Hours: </label>
              <br />
              <input
                type="text"
                className="bikedbtn"
